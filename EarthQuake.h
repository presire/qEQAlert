...
public:     // Methods
    ...
    int  UpdateInfo(const QString &fileName);                   // 発生した地震情報のログファイルに地震情報を更新する (発生日時の更新、地震IDの追加)
};
...
private:    // Methods
    int         onEQDownloaded();                                               // 緊急地震速報(警報)および発生した地震情報のデータを取得する
    int         FormattingData();                                               // 取得した地震情報を整形する
    ...
    int         DeleteObject(const QString &hypo) const;                        // ログファイルから生存していないスレッド情報を削除する
...
public:     // Methods
    ...
    explicit EarthQuake(bool bEQAlert, bool bEQInfo, int AlertScale, int InfoScale, QString AlertFile, QString InfoFile,
                        QString RequestURL, bool bSubjectTime, bool bEQchangeTitle, THREAD_INFO ThreadInfo, QObject *parent = nullptr);
    ~EarthQuake() override;
